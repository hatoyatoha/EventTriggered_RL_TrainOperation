...
class TTOPEnv(gym.Env):
    ...
    def convert_action(self, action):
        return action % 4, (action // 4 + 1) * 2

    def reward_function(self, next_state, current_time, action2, energy):
        reward1 = -0.001 * energy
        time, position, velocity = next_state

        done = bool(time >= self.planning_time and time == current_time + action2)

        reward2, reward3, reward4 = 0, 0, 0

        if done and velocity == 0:
            if position < 2000:
                reward2 = (position - 1000) / 1000
            else:
                reward2 = (3000 - position) / 1000
        ...
        return reward1 + reward2 + reward3 + reward4, done
    ...
    def _move(self, action1):
        time, position, velocity = self.state
        acceleration, force = self.accel(action1, velocity)
    ...
    def step(self, current_time, action1, action2):
        self.state, energy, acceleration = self._move(action1)
        reward, done = self.reward_function(self.state, current_time, action2, energy)
        norm_state = self.normalize_state()
        return norm_state, reward, done, {"energy": energy, "action": (action1, action2), "accel": acceleration}
